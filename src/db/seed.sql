...
SELECT id, 'Community Tech Meetup', 'A casual meetup for local developers.', 'Tech', 'Downtown Hub', 1.290270, 103.851959, DATE_ADD(NOW(), INTERVAL 7 DAY), DATE_ADD(NOW(), INTERVAL 7 DAY) + INTERVAL 2 HOUR, 50, 'approved'
FROM users WHERE email = 'organiser@example.com';

INSERT INTO events (organiser_id, title, description, category, venue_name, lat, lng, starts_at, ends_at, capacity, status)
SELECT id, 'Weekend Art Fair', 'Local artists showcase their work.', 'Arts', 'City Park', 1.290270, 103.851959, DATE_ADD(NOW(), INTERVAL 14 DAY), DATE_ADD(NOW(), INTERVAL 14 DAY) + INTERVAL 4 HOUR, 100, 'pending'
FROM users WHERE email = 'organiser@example.com';
